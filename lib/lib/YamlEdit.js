    ...
    try {
        ...
        if (!isRemoval) {
            if (targetType === 'Group') {
                ...
                if ((type === 'White' && !Cfg.App.WhiteList.groups) || type === 'Black' && !Cfg.App.BlackList.groups)
                    e.reply(`当前${type === 'White' ? '白' : '黑'}名单中的群配置未开启，黑白名单将无效，可前往config/config/App.yaml中启用`, { at: true });
            }
            else {
                yaml.append(`${type === 'White' ? 'WhiteList.users' : 'BlackList.users'}`, String(id));
                // eslint-disable-next-line @stylistic/no-mixed-operators
                if ((type === 'White' && !config.App.WhiteList.users) || type === 'Black' && !config.App.BlackList.users)
                    e.reply(`当前${type === 'White' ? '白' : '黑'}名单中的用户配置未开启，黑白名单将无效，可前往config/config/App.yaml中启用`, { at: true });
            }
 ...
 * @returns 是否继续处理
 */
const UpdateFile = async (e, name, type, item, content) => {
    try {
